...
with col_input:
    if st.session_state.mostrar_form:
        ...
        st.subheader("💵 Conversão")  
        st.info(f"Valor atual: {st.session_state.valor_conversao_yuan:.2f} Yuans", icon="💰")          
        st.info(f"Valor atual:  {st.session_state.valor_conversao_dolar:.2f} Dólares", icon="💰")
...
with col_input:
    if st.session_state.mostrar_frete:
        ...
        st.subheader("🛬 Frete")            
        st.info(f"Frete atual: R${st.session_state.frete}")
